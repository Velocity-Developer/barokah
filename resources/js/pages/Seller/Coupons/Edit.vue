...
import { Head, Link } from '@inertiajs/vue3'; import { reactive } from 'vue'; import Heading from '@/components/Heading.vue';
const props=defineProps<{coupon:any}>(); const form=reactive({...props.coupon,maximum_discount:props.coupon.maximum_discount??'',minimum_spend:props.coupon.minimum_spend??'0',usage_limit:props.coupon.usage_limit??'',per_user_limit:props.coupon.per_user_limit??'',starts_at:props.coupon.starts_at?.slice(0,16),ends_at:props.coupon.ends_at?.slice(0,16)}); async function save():Promise<void>{await fetch(`/api/v1/seller/coupons/${props.coupon.id}`,{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':(document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.content??''},body:JSON.stringify(form)});location.href='/seller/coupons'}
</script>
<template>
    <Head title="Edit coupon" />
    <div class="flex w-full flex-col gap-4 p-4">
        <Link href="/seller/coupons">← Kembali</Link>
        <Heading variant="small" title="Edit coupon" :description="coupon.code" />
