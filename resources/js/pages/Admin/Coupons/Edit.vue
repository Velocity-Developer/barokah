...
const props=defineProps<{coupon:any}>(); const form=reactive({...props.coupon,maximum_discount:props.coupon.maximum_discount??'',minimum_spend:props.coupon.minimum_spend??'0',usage_limit:props.coupon.usage_limit??'',per_user_limit:props.coupon.per_user_limit??'',starts_at:props.coupon.starts_at?.slice(0,16),ends_at:props.coupon.ends_at?.slice(0,16)}); const error=ref('');
async function save():Promise<void>{const r=await fetch(`/api/v1/admin/coupons/${props.coupon.id}`,{method:'PUT',credentials:'same-origin',headers:{Accept:'application/json','Content-Type':'application/json','X-CSRF-TOKEN':(document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.content??''},body:JSON.stringify(form)});if(!r.ok){error.value=Object.values((await r.json()).errors??{}).flat()[0] as string??'Gagal memperbarui coupon.';return}router.visit('/admin/coupons')}
</script>
<template>
