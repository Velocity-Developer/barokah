...
function visit(overrides: Partial<typeof props.filters> = {}): void {
    ...
    router.get('/admin/coupons', clean, { preserveState: true, preserveScroll: true, replace: true });
}
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Heading variant="small" title="Coupons" description="Discount codes customers enter at checkout, for the whole marketplace or one store." />
            <Link href="/admin/coupons/create" class="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90">
        ...
        <section class="rounded-xl border bg-card shadow-sm">
            ...
            <div class="flex flex-col gap-2 p-3 lg:flex-row lg:items-center">
                <label class="relative flex-1">
                    ...
                    <input v-model="search" type="search" placeholder="Search code, name or store" class="h-9 w-full rounded-md border bg-background pr-3 pl-8 text-sm" />
                </label>
                <div class="flex flex-wrap gap-2">
                    <select :value="filters.owner" :class="selectClass" aria-label="Scope" @change="visit({ owner: ($event.target as HTMLSelectElement).value })">
            ...
            <div class="border-t">
                <div v-if="coupons.data.length === 0" class="p-10 text-center text-sm text-muted-foreground">
                    <template v-if="filters.search || filters.status || filters.owner">No coupons match these filters.</template>
                    ...
                    <table class="w-full min-w-[920px] text-left text-sm">
                        ...
                        <tbody>
                            <tr v-for="coupon in coupons.data" :key="coupon.id" class="border-b last:border-0 hover:bg-muted/50">
                                ...
                                <td class="px-4 py-2.5">{{ coupon.owner ?? 'Marketplace-wide' }}</td>
                                ...
                                <td class="px-4 py-2.5">
                                    <div class="flex items-center justify-end gap-1">
                                        <Link :href="`/admin/coupons/${coupon.id}/edit`" class="inline-flex size-8 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground" :aria-label="`Edit ${coupon.code}`">
                                            <Pencil class="size-4" />
                                        </Link>
                                        <button
                                            type="button"
