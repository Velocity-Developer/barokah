...
const props = defineProps<{
    ...
    /** Admins see every store and delete by sale; sellers see their own. */
    mode?: 'admin' | 'seller';
}>();

const isAdmin = computed(() => (props.mode ?? 'admin') === 'admin');
const routes = computed(() => (isAdmin.value ? adminRoutes : sellerRoutes));
const deleteBase = computed(() => (isAdmin.value ? '/api/v1/admin/flash-sales' : '/api/v1/seller/flash-sales'));
...
function visit(overrides: Partial<typeof props.filters> = {}): void {
    ...
    router.get(routes.value.index().url, clean, { preserveState: true, preserveScroll: true, replace: true });
}
...
async function stop(sale: Sale): Promise<void> {
    ...
    try {
        const response = await fetch(`${deleteBase.value}/${sale.id}`, {
            method: 'DELETE',
    ...
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Heading
                variant="small"
                title="Flash sales"
                :description="isAdmin ? 'Time-limited promo prices on products from every store.' : 'Give one of your products a lower price for a limited time and quantity.'"
            />
            <Link :href="routes.create()" class="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90">
        ...
        <section class="rounded-xl border bg-card shadow-sm">
            ...
            <div class="flex flex-col gap-2 p-3 sm:flex-row sm:items-center">
                <label class="relative flex-1">
                    ...
                    <input v-model="search" type="search" :placeholder="isAdmin ? 'Search product or store' : 'Search product'" class="h-9 w-full rounded-md border bg-background pr-3 pl-8 text-sm" />
                </label>
                                ...
                                <td class="px-4 py-2.5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <p v-if="isAdmin" class="text-xs text-muted-foreground">{{ sale.product?.store ?? '—' }}</p>
                                        </div>
                                    </div>
                                </td>
